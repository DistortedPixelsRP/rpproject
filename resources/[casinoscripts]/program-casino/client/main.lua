	...
	while true do
		...
		for i,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then
				alert("Entrar al casino")

				if IsControlJustReleased(1, 38) then
					if IsPedInAnyVehicle(player, true) then
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Entrada Clasica",  value = 'classicTicket'},
							{label = "Miembro Gold",  value = 'goldTicket'},
							{label = "Miembro Platino", value = "platiniumTicket"},
							{label = "Trabajadores", value = "worker"}
						}
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CasinoCloak', {
							...
							elseif data.current.value == 'worker' then
								if PlayerData.job.name=='casino' then
									...
									menu.close()
									ESX.ShowNotification('~r~Usted no es un empleado del Casino.')
								end
							end
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CasinoCloak', {
							...
							elseif data.current.value == 'worker' then
								if PlayerData.job.name=='casino' then
									...
									menu.close()
									ESX.ShowNotification('~rUsted no es un empleado del Casino.')
								end
								wyzeruj()
							end
...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='CasinoActions' then
		...
		CurrentActionMsg  = "~r~[E] ~b~Abre el menú de gestión del casino."
		CurrentActionData = {}
	elseif zone == "CasinoCloak" then
		CurrentAction     = 'CasinoCloak'
		CurrentActionMsg  = "~r~[E] ~p~Cambiador"
		CurrentActionData = {}
	elseif zone == "CasinoFridge" then
		CurrentAction     = 'CasinoFridge'
		CurrentActionMsg  = "~r~[E] ~p~Heladera"
		CurrentActionData = {}
...
AddEventHandler('program-casino:openTicketMenuClient', function(worker)
	...
	if platiniumTicket then
		table.insert(elements, {label = "Mostrar boleto platino", value = 'showPlatinium'})
	end
	if classicTicket then
		table.insert(elements, {label = "Mostrar el boleto", value = 'showClassic'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pokazTicket', {
		...
		elseif data.current.value == 'showGolden' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene un boleto de oro", worker)
		elseif data.current.value == 'showPlatinium' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene un boleto de platino.", worker)
		elseif data.current.value == 'showClassic' then
			TriggerServerEvent('program-casino:sendTicket', "El cliente tiene un ticket clásico.", worker)
		end
		...
		function(data, menu)
		  ...
		  ESX.UI.Menu.Open(
			...
			function(data2, menu2)
			  ...
			  if count == nil then
				ESX.ShowNotification("Cantidad Erronea")
			  else
				menu2.close()
				menu.close()
				OpenGetFridgeStocksMenu()
...
function OpenVaultMenu()
	...
	local elements = {
		{label = "Toma el arma", value = 'get_weapon'},
		{label = "Guarda el arma", value = 'put_weapon'},
		{label = "Tomar artículos", value = 'get_stock'},
		{label = "Guarda los artículos", value = 'put_stock'}
	}
