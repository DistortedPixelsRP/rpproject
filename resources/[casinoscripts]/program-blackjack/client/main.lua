...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='Croupier' then
		...
		CurrentActionMsg  = "Presiona ~r~[E] ~b~ para abrir la mesa de blackjack"
		CurrentActionData = {}
	elseif zone == "Player1" then
		CurrentAction     = 'Player1'
		CurrentActionMsg  = "Presiona ~r~[E] ~b~ para unirte como primer jugador"
		CurrentActionData = {}
	elseif zone == "Player2" then
		CurrentAction     = 'Player2'
		CurrentActionMsg  = "Presiona ~r~[E] ~b~ para unirte como segundo jugador"
		CurrentActionData = {}
	end
end)
...
AddEventHandler('program-casino:notifyPlayers', function(quantity)
	...
	if GetDistanceBetweenCoords(coords, Config.BlackjackSites.Croupier.Pos.x, Config.BlackjackSites.Croupier.Pos.y, Config.BlackjackSites.Croupier.Pos.z, true) < 29 then
		ESX.ShowAdvancedNotification("~p~BlackJack", "~p~Stawka: ~g~"..quantity.."$", "~p~ La mesa comenzó, recuerde que el Boleto Golden / Platinium tiene prioridad", "CHAR_BANK_BOL", 9)
	end
	Citizen.Wait(5000)
end)
...
AddEventHandler('program-blackjack:givecards', function()
	...
	local finish = false
	ESX.ShowNotification("~p~Te uniste a la mesa, tu primera carta es ~b~" ..cards[card][1].. " ~p~Valores ~b~"..cards[card][2].."~n~~b~" ..wynik.." ~p~pkt")
	ESX.ShowNotification("~o~ Para seleccionar la carta, haga click ~r~ [F]~o~ Para interrumpir y sacar las cartas, haga click en ~r~[X]")
		...
		if IsControlJustReleased(0, Keys["F"]) then
			...
			if wynik>21 then
				if aces > 0 then
					aces = aces-1
					wynik = wynik - 10
					ESX.ShowNotification("~p~Tenías As, por eso no perdiste, solo te contamos 1 punto del as en lugar de 11")
				else
					finish=true
					ESX.ShowNotification("~p~En total, tienes más de 21 puntos, es decir ~b~" ..wynik.. "~p~, Perderás a menos que el oponente también haya superado 21.")
					aces = 0
				end
			end
...
function Croupier()
	...
	elseif PlayerData.job.name ~= 'casino' then
		ESX.ShowNotification("Debes ser un croupier para abrir la mesa.")
	elseif gameInProgress == true and PlayerData.job.name == 'casino' then
		ESX.ShowNotification("~b~Actualmente, el juego está en progreso para iniciar el nuevo de forma remota Presiona ~r~[X]~b~ o espera hasta el final")
	end
...
function Player1()
	ESX.TriggerServerCallback('program-blackjack:checkStarting', function(result)
		if not result then
			ESX.ShowNotification('~r~La mesa no se ha quitado, ¡ve al distribuidor para pedir la mesa!')
		else
			if isPlatinium then
				local elements = {
