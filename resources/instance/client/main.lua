...
Citizen.CreateThread(function()
	while true do
		...
		if InstanceInvite ~= nil and IsControlJustReleased(0, Keys['E']) then
			...
			NetworkSetVoiceChannel(InstanceInvite.host)
			...
			for i=0, Config.MaxPlayers, 1 do
				...
				if NetworkIsPlayerActive(i) then
					...
					--[[if not found then
						...
						SetPedCanRagdoll(otherPlayerPed, false)
						ClearPedBloodDamage(otherPlayerPed)
						ResetPedVisibleDamage(otherPlayerPed)
						ClearPedLastWeaponDamage(otherPlayerPed)
						SetEntityProofs(otherPlayerPed, true, true, true, true, true, true, true, true)
						--SetEntityOnlyDamagedByPlayer(otherPlayerPed, false)
						--SetEntityCanBeDamaged(otherPlayerPed, false)

						-- Invisible
						SetEntityVisible(otherPlayerPed, false, false)
						SetEntityCollision(otherPlayerPed, false, false)
						FreezeEntityPosition(otherPlayerPed, true) 
						SetBlockingOfNonTemporaryEvents(otherPlayerPed, true)


						--SetEntityAlpha(otherPlayerPed, 0, 0)
						--SetEntityLocallyInvisible(otherPlayerPed)
						--SetEntityNoCollisionEntity(playerPed, otherPlayerPed, true)
					else 
						local otherPlayerPed = GetPlayerPed(i)
						...
						SetPedCanRagdoll(otherPlayerPed, true)
						ClearPedLastWeaponDamage(otherPlayerPed)
						SetEntityProofs(otherPlayerPed, false, false, false, false, false, false, false, false)
						--SetEntityOnlyDamagedByPlayer(otherPlayerPed, true)
						--SetEntityCanBeDamaged(otherPlayerPed, true)

						-- Invisible
						SetEntityVisible(otherPlayerPed, true, true)
						SetEntityCollision(otherPlayerPed, true, true)
						FreezeEntityPosition(otherPlayerPed, false) 
						SetBlockingOfNonTemporaryEvents(otherPlayerPed, false)
