...
function bedActive(x, y, z, heading)
    ...
    --    if (GetEntityHealth(GetPlayerPed(-1)) < 200)  then
      --      TriggerEvent('esx:showNotification', '~g~Estas siendo atendido, por favor espera.'); 
        --    Citizen.Wait(35000) -- Time before heal
          --  SetEntityHealth(PlayerPedId(-1), 200)
            --TriggerEvent('esx:showNotification', '~g~ Te han curado!'); 

        --elseif (GetEntityHealth(GetPlayerPed(-1)) == 200) then
         --   TriggerEvent('esx:showNotification', '~y~ No necesitas atencion medica, tu salud se encuentra bien'); 
        --end
    --end)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if InAction == true then
                DrawSub('Presiona ~g~Z~w~ Para levantarse')
