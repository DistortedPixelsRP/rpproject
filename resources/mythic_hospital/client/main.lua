...
local pillboxTeleports = {
    { x = 325.48892211914, y = -598.75372314453, z = 43.291839599609, h = 64.513374328613, text = 'Presiona ~INPUT_CONTEXT~ ~s~para ir a la entrada' },
    { x = 355.47183227539, y = -596.26495361328, z = 28.773477554321, h = 245.85662841797, text = 'Presiona ~INPUT_CONTEXT~ ~s~para entrar al Hospital' },
    { x = 359.57849121094, y = -584.90911865234, z = 28.817169189453, h = 245.85662841797, text = 'Presiona ~INPUT_CONTEXT~ ~s~para entrar al Hospital' },
}
...
AddEventHandler('mythic_hospital:client:RPSendToBed', function(id, data)
    ...
    Citizen.CreateThread(function()
        while bedOccupyingData ~= nil do
            Citizen.Wait(1)
            PrintHelpText('Presiona ~INPUT_VEH_DUCK~ para levantarte')
...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    Citizen.CreateThread(function ()
        ...
        exports['mythic_notify']:DoHudText('inform', 'Los doctores te estan tratando')
        Citizen.Wait(5000)
...
AddEventHandler('mythic_hospital:client:FinishServices', function()
    ...
    exports['mythic_notify']:DoHudText('inform', 'Haz\'sido tratado y facturado')
    LeaveBed()
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
			...
			ESX.Game.Utils.DrawText3D(vector3(hospitalCheckin.x, hospitalCheckin.y, hospitalCheckin.z + 0.5), '[E] Registrarse', 0.4)
                        if IsControlJustReleased(0, 54) then
                            if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                                ...
                                exports['mythic_notify']:DoHudText('error', 'No necesitas atencion medica')
                            end
                        end
                    end
                end
