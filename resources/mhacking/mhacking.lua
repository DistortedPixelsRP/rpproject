	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Muevete con ~y~W,A,S,D~s~ y confirma con ~y~SPACE~s~ para el codigo de la izquierda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utiliza ~y~Arrow Keys~s~ y ~y~ENTER~s~ para el codigo de la derecha.")
			else
