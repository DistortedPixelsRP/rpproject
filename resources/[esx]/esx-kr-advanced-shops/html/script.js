...
$(function() {
		window.addEventListener('message', function(event) {
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Jefe Acciones</button>
					`
				);
			}
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<h6>${cb[i].label}</h4>
							<h6>$${cb[i].price} por articulo</h4>
							<h6>En stock: ${cb[i].count}</h4>
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Cuantos?"></textarea>
							</div>`
								);
							};
							...
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">Atras</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$.post('http://esx-kr-advanced-shops/notify', JSON.stringify({msg : "~r~Uno de los artículos no tiene stock suficiente o la cantidad no es válida."}));
					}
				}
			});
