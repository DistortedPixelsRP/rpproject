...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Tu: $' .. data[1].money .. ' en la empresa',    value = ''})
		table.insert(elements, {label = 'Envios',    value = 'shipments'})
		table.insert(elements, {label = 'Poner en un artículo para la venta', value = 'putitem'})
		table.insert(elements, {label = 'Sacar un articulo de la venta',    value = 'takeitem'})
		table.insert(elements, {label = 'Depositar dinero en tu empresa.',    value = 'putmoney'})
		table.insert(elements, {label = 'Sacar dinero de tu empresa.',    value = 'takemoney'})
		table.insert(elements, {label = 'Cambiar nombre de tu empresa: $' .. Config.ChangeNamePrice,    value = 'changename'})
		...
		elseif data.current.value == 'sell' then
		  ...
		  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
			title = 'ESCRIBIR: (SÍ) sin paréntesis para confirmar'
	  ...
	  elseif data.current.value == 'changename' then
		...
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'changename', {
		  title = 'Cómo te gustaría que sea el nombre de tu tienda?'
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Pedir producto', value = 'buy'})
	table.insert(elements, {label = 'Los envios', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '--ENVIOS LISTOS--'})
			table.insert(elements, {label = 'Agarrar todos los envios', value = 'pickup'})
			once2 = false
			end
...
function TakeItem(number)
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getShopItems', function(result)
	...
	for i=1, #result, 1 do
	    if result[i].count > 0 then
	    	table.insert(elements, {label = result[i].label .. ' | ' .. result[i].count ..' piezas en el deposito [' .. result[i].price .. ' $ por pieza', value = 'removeitem', ItemName = result[i].item})
	    end
    end
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
                    ...
                    local coords = {
						...
						TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~r~Tu tienda ~b~(" .. result.name .. ')~r~ esta bajo robo', id)
					...
					ESX.ShowNotification("~r~No hay suficientes políticas en línea. " .. results .. '/' .. Config.RequiredPolices)
				end
			else
				ESX.ShowNotification("~r~Esta tienda ya ha sido robada, por favor espere. " ..  math.floor((Config.TimeBetweenRobberies - result.time)  / 60) .. ' minutes')
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 3 then
							if IsPedUsingScenario(GetPlayerPed(-1), "WORLD_HUMAN_CONST_DRILL") then
									SetEntityHealth(Var, (GetEntityHealth(Var) - 1))
							end
					end
					local hp = GetEntityHealth(Var)
					hp = math.floor(hp)
					TriggerEvent("mt:missiontext", "Romper la bóveda:~r~ " .. hp/100 .. "%", 1000)

				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('esx_kr_shops-robbery:GetReward', Id)
					TriggerServerEvent("esx_kr_shops-robbery:NotifyOwner", '~r~El robo en tu tienda ~b~(' .. Name ..')~r~ desafortunadamente fue exitoso!', Id)
					DeleteEntity(Var)
				end
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~g~El robo en tu tienda ~b~(" .. Name .. ')~g~ no tuvo éxito!', Id)
