  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        function(data)
            ...
            if count > data[1].count then
                ...
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~No puedes sacar más de lo que tienes')
                else
      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Ya tienes un mismo artículo en tu tienda., ~r~pero por ' .. data[1].price .. '. pones el precio ' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Quitar el artículo y poner un nuevo precio o poner el mismo precio')
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('esx:showNotification', src, '~r~No tienes suficiente dinero.')
            elseif ItemCount <= 0 then
                TriggerClientEvent('esx:showNotification', src, '~r~cantidad inválida.')
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Tú compraste ' .. ItemCount .. ' x ' .. Item .. ' por $' .. ItemCount * result[1].price)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Usted ordeno' .. count .. ' piezas ' .. label .. ' por $' .. price * count)
        else
            TriggerClientEvent('esx:showNotification', _source, '~r~no tienes suficiente dinero en tu tienda.')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Compraste una tienda por $' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~Usted no puede pagar esta tienda')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~El dinero de su tienda ha sido bloqueado debido a un robo, por favor espere ' .. math.floor((900 - time) / 60) .. ' minutes')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Pones $' .. amount .. ' en tu tienda')
        else
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~No puedes poner más de lo que tienes')
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Tu sacaste $' .. amount .. ' de tu tienda')
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Vendiste tu tienda')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~No puedes vender tu tienda con artículos o dinero dentro de ella.')
            end
        end)
    end)
end)
