        ...
        SetTimeout(10000, function()        
            ...
            elseif daletQuantity < 2 then
                ...
                if chanceToMiss <= Config.MissCraft then
                    ...
                    TriggerClientEvent('esx:showNotification', _source, _U('craft_cocktail') .. _U('mixapero') .. ' ~w~!')
                    xPlayer.removeInventoryItem('bolcacahuetes', 2)
                    xPlayer.removeInventoryItem('bolnoixcajou', 2)
                    xPlayer.removeInventoryItem('bolpistache', 2)
        ...
        SetTimeout(10000, function()        
            ...
            elseif gimelQuantity < 2 then
                ...
                if chanceToMiss <= Config.MissCraft then
                    ...
                    TriggerClientEvent('esx:showNotification', _source, _U('craft_cocktail') .. _U('fernetcola') .. ' ~w~!')
                    xPlayer.removeInventoryItem('cocacola', 1)
                    xPlayer.removeInventoryItem('ice', 2)
                    xPlayer.removeInventoryItem('fernet', 2)
...
AddEventHandler('esx_restaurantejob:craftingFood', function(itemValue)
    ...
    if _itemValue == 'lasagna' then
        SetTimeout(10000, function()
            ...
            local alephQuantity     = xPlayer.getInventoryItem('harina').count
            local bethQuantity      = xPlayer.getInventoryItem('tomate').count
            local gimelQuantity     = xPlayer.getInventoryItem('picada').count
            local fourthQuantity    = xPlayer.getInventoryItem('acelga').count

            if alephQuantity < 1 then
                TriggerClientEvent('esx:showNotification', _source, _U('No tiene suficientes') .. _U('harinas') .. '~w~')
            elseif bethQuantity < 3 then
                TriggerClientEvent('esx:showNotification', _source, _U('No tiene suficientes') .. _U('tomates') .. '~w~')
            elseif gimelQuantity < 1 then
                TriggerClientEvent('esx:showNotification', _source, _U('No tiene suficientes') .. _U('carnes picadas') .. '~w~')
            elseif fourthQuantity < 2 then
                TriggerClientEvent('esx_showNotificacion', _source, _U('Notiene suficientes') .. _U('acelgas') .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
                    TriggerClientEvent('esx:showNotification', _source, _U('craft_miss_food'))
                    xPlayer.removeInventoryItem('harina', 1)
                    xPlayer.removeInventoryItem('tomate', 3)
                    xPlayer.removeInventoryItem('picada', 1)
...
AddEventHandler('esx_restaurante:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'police' then
		print(('esx_restaurante: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
