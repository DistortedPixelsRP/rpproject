...
function OpenSocietyActionsMenu()
  ...
  if (isBarman or IsGradeBoss()) then
    ...
    table.insert(elements, {label = _U('MusicaON'),    value = 'MusicaON'}) 
    table.insert(elements, {label = _U('MusicaOFF'),    value = 'MusicaOFF'})
  end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'MusicaON' then
        TriggerEvent('esx_jb_dj:enabledjbooth', true)
         ESX.ShowNotification('Encendiste la Musica')
       end
             if data.current.value == 'MusicaOFF' then
        TriggerEvent('esx_jb_dj:enabledjbooth', false)
         ESX.ShowNotification('Apagaste la Musica')
       end
