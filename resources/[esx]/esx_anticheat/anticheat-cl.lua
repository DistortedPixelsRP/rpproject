			...
			for ped in thePeds do
				...
				if PedStatus >= Config.pedThreshold then
					if not (IsPedAPlayer(ped))then
						if not IsThisModelPedAllowed(ped) then
							DeleteEntity(ped)
...
Citizen.CreateThread( function()
	while true do
        ...
        if whitelisted == true then
			rgb = RGBRainbow(2)
			ACstatus(0.18, 0.85, 1.0,1.0,0.4, "AntiCheat: "..AntiCheatStatus, rgb.r, rgb.g, rgb.b, 255, 200)
			ACstatus(0.18, 0.85 + 0.03, 1.0,1.0,0.35, "NPCs: ~r~"..PedStatus, 255, 255, 255, 200)
		end
	end
end)
...
function IsThisModelPedAllowed(ent)
	local model = GetEntityModel(ent)

	for i = 1, #Config.AllowPeds do
		if model == GetHashKey(Config.AllowPeds[i]) then
			return true
		end
	end
	return false
