...
AddEventHandler('esx_cityworks:GiveItem', function()
	...
	if Quantity >= 15 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Has ~g~completado la reparacion. Ve al siguiente punto')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~No tienes mas facturas para cobrar')
				...
				TriggerClientEvent('esx:showNotification', _source, 'Obtuviste: ~g~$' .. Config.Zones.Vente.ItemPrice)
				Vente(_source)
			end
...
AddEventHandler('esx_cityworks:startVente', function()
	...
	if PlayersVente[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~No hay facturas para pagar')
		PlayersVente[_source] = false
	else
