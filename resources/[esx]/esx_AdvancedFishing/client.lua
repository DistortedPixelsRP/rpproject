...
local blip = AddBlipForCoord(Config.SellFish.x, Config.SellFish.y, Config.SellFish.z)
			...
			AddTextComponentString("Venta de pezcado")
...
for _, info in pairs(Config.MarkerZones) do
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Renta de Barcos")
		...
		if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~El Pez se ha escapado")
				end
			end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~El pez se esta acercando \n ~h~Presiona " .. correct .. " para atraparlo!")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~No puedes pescar desde un vehiculo")
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			...
			ESX.ShowNotification("~y~Necesitas ir más lejos de la costa")
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Presiona E Para alquilar un Barco')
...
function OpenBoatsMenu(x, y , z)
	...
	local elements = {}
		...
		table.insert(elements, {label = '<span style="color:green;">Dinghy</span> <span style="color:red;">$1500</span>', value = 'boat'})
		table.insert(elements, {label = '<span style="color:green;">Toro</span> <span style="color:red;">$3000</span>', value = 'boat2'}) 
		table.insert(elements, {label = '<span style="color:green;">Suntrap</span> <span style="color:red;">$5000</span>', value = 'boat6'}) 
		table.insert(elements, {label = '<span style="color:green;">Marquis</span> <span style="color:red;">$6000</span>', value = 'boat3'}) 
		table.insert(elements, {label = '<span style="color:green;">Tug boat</span> <span style="color:red;">$6000</span>', value = 'boat4'})
		table.insert(elements, {label = '<span style="color:green;">Jetmax</span> <span style="color:red;">$6000</span>', value = 'boat5'}) 	
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un Barco por', {255,0,255}, '$' .. 1500)
	...
	if data.current.value == 'boat2' then
		...
		TriggerServerEvent("fishing:lowmoney", 3000) 
		TriggerEvent("chatMessage", 'Alquilaste un Barco por', {255,0,255}, '$' .. 3000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un Barco por', {255,0,255}, '$' .. 5000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerEvent("chatMessage", 'Alquilaste un Barco por', {255,0,255}, '$' .. 6000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Sacaste un Barco')
		SetPedCoordsKeepVehicle(ped, x, y , z)
