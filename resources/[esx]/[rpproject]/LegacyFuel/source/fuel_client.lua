...
function ManageFuelUsage(vehicle)
	if not DecorExistOn(vehicle, Config.FuelDecor) then
		if PlayerData.job.name == 'fueler' then
			SetFuel(vehicle, math.random(900, 900) / 10)
		else
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	if pumpObject then
		TriggerServerEvent('fuel:pay', currentCost, currentZone)
...
function fuelCharge(stringCoords, ped)
				...
				if currentCash >= Config.JerryCanCost then
					if not HasPedGotWeapon(ped, 883325847) then
						...
						if IsControlJustReleased(0, 38) then
							...
							TriggerServerEvent('fuel:pay', Config.JerryCanCost, currentZone)
						...
						if Config.UseESX then
							...
							if refillCost > 0 then
								if currentCash >= refillCost then
									...
									if IsControlJustReleased(0, 38) then
										TriggerServerEvent('fuel:pay', refillCost, currentZone)
...
Citizen.CreateThread(function()
    while true do
        ...
        if currentZone ~= nil and isNearPump then
            if fuelStock > 0 then
                fuelCharge(stringCoords, ped)
            else
                DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, "Esta gasolinera no tiene stock")
            end
        else
            fuelCharge(stringCoords, ped)
        end
...
Citizen.CreateThread(function()
	while true do
		...
		Citizen.Wait(10)
			...
			for k,v in pairs(Config.Marcadores) do
	...
	while true do
		...
		Citizen.Wait(10)
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('legacyfuel:hasEnteredMarker', currentZone)
			end
  
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('legacyfuel:hasLeftMarker', currentZone)
			end
...
RegisterNetEvent('fuel:getClientStock')
AddEventHandler('fuel:getClientStock', function(zone)
	ESX.TriggerServerCallback('fuel:getStock', function(response)
		fuelStock = response
	end, zone)
end)

-- evento cuando está dentro del marcador
AddEventHandler('legacyfuel:hasEnteredMarker', function(zone)
	TriggerEvent('fuel:getClientStock', zone)
end)

AddEventHandler('legacyfuel:hasLeftMarker', function(zone)
	--Citizen.Trace("No está en un punto de combustible")
	currentZone = nil
end)