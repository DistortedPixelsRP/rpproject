...
RegisterNUICallback('NUIservjobActions', function()
  SetNuiFocus(true, true)
  SendNUIMessage({type = 'openServJob'})
end)

RegisterNUICallback('NUIActionService', function(data, cb)
	...
	DisplayOnscreenKeyboard(1, "FMMC_KEY_TIP8", "", "", "", "", "", 240)
  while (UpdateOnscreenKeyboard() == 0) do
	...
	elseif data.service == 'taxi' then
		messageSend = ''
		messageNotif = '¡La señal ha sido enviada a las unidades disponibles!'
	elseif data.service == 'mechanic' then
		messageSend = ''
		messageNotif = '¡La señal ha sido enviada a las unidades disponibles!'
	elseif data.service == 'fire' then
		messageSend = ''
		messageNotif = '¡La señal ha sido enviada a las unidades disponibles!'
	else
		messageSend = ''
	...
	TriggerServerEvent("call:makeCall", data.service, PlayerCoords, messageSend, GetPlayerServerId(player))
	
	--TriggerServerEvent('esx_addons_gcphone:startCall', data.service, messageSend, PlayerCoords, {
