local function commandHandler(command, source, args, raw)
    ...
    if #args > 0 or command.noargs then
        ...
        local visualUsername = GetPlayerName(source):gsub('%^', '')
        ...
        local colorcustom = command.color
        TriggerEvent('rpproject_vip:getData', source, function (userdata)
            if userdata ~= nil then
                if userdata.id_grado == 4 then
                    visualName = '💎' ..visualName .. '💎'
                    visualUsername =  '💎' ..visualUsername .. '💎'
                    colorcustom = {185,242,255}
               elseif userdata.id_grado == 3 then
                    visualName = '🥇' ..visualName .. '🥇'
                    visualUsername =  '🥇' ..visualUsername .. '🥇'
                    colorcustom = {255,223,0}
                elseif userdata.id_grado == 2 then
                    visualName = '🥈' ..visualName .. '🥈'
                    visualUsername =  '🥈' ..visualUsername .. '🥈'
                    colorcustom = {192,192,192}
               end
            end
        ...
        if not command.hidden then
            ...
            if command.range and command.range ~= -1 then
                ...
                TriggerClientEvent("chat_commands:proximity", -1, source, command.range, {
                    ...
                    color = colorcustom or {255, 255, 255},
                    multiline = true,
                    args = messageArgs
                })
            else
