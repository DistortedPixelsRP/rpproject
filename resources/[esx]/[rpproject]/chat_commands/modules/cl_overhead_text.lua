...
AddEventHandler("chat_commands:showFloatingText", function(target, text)
    table.insert(FloatingText, {target = GetPlayerFromServerId(target), text = tostring(text), life = GetGameTimer() + FloatingTextDuration})
end)
    ...
    while true do
        ...
        for n, text in next, FloatingText do
            if NetworkIsPlayerActive(text.target) then
                ...
                local pos = GetEntityCoords(ply, true)
                if #(lpos - pos) <= FloatingTextMaxDistance then
                    DrawText3D(text.text, pos.x, pos.y, pos.z + 1.2)
                end
            else
                -- Offline player
            end
