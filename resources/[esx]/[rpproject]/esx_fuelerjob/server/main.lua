	...
	if PlayersSelling[source] == true then
		...
		if zone ~= nil then
			if xPlayer.getInventoryItem('essence').count <= 0 then
				...
				essence = 1
				essenceCount = xPlayer.getInventoryItem('essence').count
			end
			...
			elseif xPlayer.getInventoryItem('essence').count <= 0 then
				...
				if (essence == 1) then
					SetTimeout(1100, function()
						local money = Config.precioFuel * essenceCount
						xPlayer.removeInventoryItem('essence', essenceCount)
						...
						if societyAccount ~= nil then
							...
							TriggerEvent('fuel:reStock', source, zone, essenceCount)
							essenceCount = 0
						end
						Sell(source,zone)
					end)
