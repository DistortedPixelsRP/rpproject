...
function hasAllIngredients(inventory, ingredients)
	for i=1, #ingredients.ingredients, 1 do
		for j=1, #inventory, 1 do
			if ingredients.ingredients[i].item == inventory[j].name then
				if inventory[j].count < ingredients.ingredients[i].quantity then
					return false
				end
			end
		end
...
AddEventHandler('salty_crafting:craftItem', function(itemname)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	if xPlayer ~= nil then
		if hasAllIngredients(xPlayer.inventory, Config.Recipes[itemname]) then
			for _,ingredient in pairs(Config.Recipes[itemname].ingredients) do
			...
			TriggerClientEvent('esx:showNotification', _source, '~y~Item crafteado ~w~')
		else
			TriggerClientEvent('esx:showNotification', _source, 'No tienes todos los ingredientes.')
		end
	end
