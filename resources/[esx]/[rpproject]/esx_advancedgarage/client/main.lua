			...
			if valid then
				if engineHealth < 990 then
					if Config.UseDamageMult then
						local apprasial = math.floor((1000 - engineHealth)/1000*Config.CarPoundPrice*Config.DamageMult+9800)
						reparation(apprasial, vehicle, vehicleProps)
					else
						local apprasial = math.floor((1000 - engineHealth)/1000*Config.CarPoundPrice+9800)
						reparation(apprasial, vehicle, vehicleProps)
					end
				else
		...
		for _,v in pairs(ownedCars) do
			if Config.UseVehicleNamesLua then
				...
				for k,v in pairs(carInWorld) do
					if all_trim(v.plate) == all_trim(plate) then 
						isSpawned = 0
...
function putaway(vehicle, vehicleProps)
	for k,v in pairs(carInWorld) do
		if all_trim(v.plate) == all_trim(vehicleProps.plate) then
			table.remove(carInWorld, k)
		end
	end
	
	-- sonny esx_jb_stopvehicledespawn
	...
	TriggerServerEvent('esx_advancedgarage:unloadCarsForPlayer')
...
function SpawnVehicle(vehicle, plate)
	ESX.Game.SpawnVehicle(vehicle.model, {
		...
		--NetworkRegisterEntityAsNetworked(VehToNet(callback_vehicle))
		--AddVehicleKeys(callback_vehicle)
		table.insert(carInWorld, {vehicleentity = callback_vehicle, plate = carplate, netID = netID})
	end)
...
RegisterNetEvent('esx_advancedgarage:Disconnect')
AddEventHandler('esx_advancedgarage:Disconnect', function(source)
	ESX.TriggerServerCallback('esx_advancedgarage:getServercars', function(serverCars)
		for k,v in pairs(serverCars) do
			local playerID = GetPlayerFromServerId(k)
			if not NetworkIsPlayerActive( playerID ) then
				for a,t in pairs(v) do
					local entityFromNetID = NetworkGetEntityFromNetworkId(t.netID)
					--Citizen.Trace(k.. ":" .. t.vehicleentity .. " | " .. entityFromNetID)
					ESX.Game.DeleteVehicle(entityFromNetID)
					DeleteEntity(entityFromNetID)
					Citizen.Trace(t.plate)
					TriggerServerEvent('esx_advancedgarage:setVehicleState', t.plate, "1")
					TriggerServerEvent('esx_jb_stopvehicledespawn:vehicleenteredingarage', netID)
				end
				TriggerServerEvent('esx_advancedgarage:unloadCarsForPlayer',k)
			end
		end
...
RegisterNetEvent('esx_advancedgarage:testing')
AddEventHandler('esx_advancedgarage:testing', function(source)
	ESX.TriggerServerCallback('esx_advancedgarage:getServercars', function(serverCars)
		print(json.encode(serverCars))

		for k,v in pairs(serverCars) do
			--Citizen.Trace(k.. ":" .. json.encode(v))
			local playerID = GetPlayerFromServerId(k)
			if not NetworkIsPlayerActive( playerID ) then
				for a,t in pairs(v) do
					local entityFromNetID = NetworkGetEntityFromNetworkId(t.netID)
					...
					--DeleteEntity(entityFromNetID)
					Citizen.Trace(t.plate)
					TriggerServerEvent('esx_advancedgarage:setVehicleState', t.plate, "3")
					TriggerServerEvent('esx_jb_stopvehicledespawn:vehicleenteredingarage', netID)
				end
				TriggerServerEvent('esx_advancedgarage:unloadCarsForPlayer',k)
			end
	...
	while true do
		...
		for _, player in ipairs(GetActivePlayers()) do
			...
			if playerPed and playerPed ~= -1 then -- Checks if the player exists
				TriggerServerEvent('updateCarsOnServerForPlayer', { table.unpack(carInWorld) })
			end
			-- do stuff
		end
		Citizen.Wait(5000)
...
function AddVehicleKeys(vehicle)
    ...
    if localVehPlateTest ~= nil then
		local localVehPlate = all_trim(tostring(localVehPlateTest))
