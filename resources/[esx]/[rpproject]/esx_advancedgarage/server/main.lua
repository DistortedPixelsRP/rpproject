...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedCarsOut', function(source, cb)
	local ownedCars = {}
...
function unloadCarsForPlayer(player)
	local _player = tonumber(player)
	ServerCars[player] = nil
	--table.remove(ServerCars, player)
end

RegisterServerEvent('esx_advancedgarage:unloadCarsForPlayer')
AddEventHandler('esx_advancedgarage:unloadCarsForPlayer', function(player)
	if player ~= nil and  ServerCars[player] ~= nil then
		unloadCarsForPlayer(player)
	end
end)

RegisterServerEvent('updateCarsOnServerForPlayer')
AddEventHandler('updateCarsOnServerForPlayer', function(cars)
	ServerCars[source] = cars
end)

AddEventHandler('playerDropped', function(player)
    --unloadCarsForPlayer(source)
	--print('Position cleared for player ' .. player .. ' with source ' .. source)
	TriggerClientEvent("esx_advancedgarage:Disconnect", -1, ServerCars[source])
end)

ESX.RegisterServerCallback('esx_advancedgarage:getServercars', function(source, cb)
	local _source = source
	cb(ServerCars)
end)

TriggerEvent('es:addGroupCommand', 'testing', 'admin', function(source, args, user)
	local _source = source
	--unloadCarsForPlayer(source)
	--TriggerClientEvent("esx_advancedgarage:testing", source, ServerCars[source])

end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end, {help = "testing", params = {} })
