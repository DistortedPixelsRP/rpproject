...
local function craftItem(ingredients)
	...
	TriggerServerEvent('salty_empaque:craftItem', ingredientsPrepped)
end

RegisterNetEvent('salty_empaque:openMenu')
AddEventHandler('salty_empaque:openMenu', function(playerInventory)
	...
	Citizen.CreateThread(function()
		while true do
			...
			if IsControlJustReleased(1, Config.Keyboard.keyCode) and GetLastInputMethod(2) then
				TriggerServerEvent('salty_empaque:getPlayerInventory')
			end
		end
	end)
end
