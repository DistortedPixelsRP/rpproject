...
function saveVehicleToFile(id, netid, model, x, y, z, heading, vehicleProps)
	local dir = dirfolder .. filename
	
	local vehiclestable = LoadVehiclesFile() 
	local netid = tostring(netid)
	vehiclestable[netid] = {model=model,ownerid=id, x=x, y=y, z=z, heading=heading, vehicleProps=vehicleProps}
