...
AddEventHandler('esx_jb_stopvehicledespawn:savevehicle', function(id, netid,model, x, y, z, heading, vehicleProps)
	local vehiclestable = LoadVehiclesFile()
	saveVehicleToFile(id, netid,model, x, y, z, heading, vehicleProps)
	--print("test1")
end)
...
AddEventHandler("esx_jb_stopvehicledespawn:vehicleenteredingarage", function(networkid)
	local _source = source
	local vehiclelist = LoadVehiclesFile()
	local networkid = tostring(networkid)
	...
	if vehiclelist[vehicleid] ~= nil then
		for i = 1, #playervehiclelist do
			...
			if string.upper(plate) == string.upper(vehiclelist[vehicleid].vehicleProps.plate) then
				...
				--print("***********")
				--print("se Borro para: " .. owner)
				--print("Patente auto: " .. plate)
				--print("***********")
				MySQL.Async.execute(
					"UPDATE owned_vehicles set stored = 3 where stored = 0 AND plate = @plate",
					{
						['@identifier'] = owner,
						['@plate'] = plate
					}
				...
				--		['@target_type'] = 'player',
				--		['@target']      = 'society',
				--		['@label']       = "Multa: Abandono de Vehiculo",
				--		['@amount']      = 3000
				--	},
