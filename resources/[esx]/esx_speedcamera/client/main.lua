...
local blips = {
	...
	{title="Speedcamera (60KM/H)", colour=1, id=1, x = -148.29, y = -825.55, z = 31.13}, -- 60KM/H ZONE
	-- 80KM/H ZONES
							...
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (60KM/H)', 2500) -- Sends a bill from the police
									else
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then					
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									TriggerEvent("pNotify:SendNotification", {text = "Te han atrapado excedido de velocidad en una zona de 80 KMH", type = "error", timeout = 5000, layout = "centerLeft"})
									
									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (80KM/H)', 3500) -- Sends a bill from the police
									else
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then 
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if useBilling == true then
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'society_police', 'Speedcamera (120KM/H)', 5000) -- Sends a bill from the police
									else
