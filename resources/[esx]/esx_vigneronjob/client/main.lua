...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Vehiculo de Trabajo',  value = 'bison3'},
		}
...
RegisterNetEvent('esx_vigneronjob:waitTime')
AddEventHandler('esx_vigneronjob:waitTime', function(qtty, action, item)
	...
	while aux <= qtty and doing == true do
		...
		if index % 180 == 0 then
			if action == 'harvest' then
				ESX.ShowNotification(_U('you_do_raisin') .. aux)
			elseif action == 'process_wine' then
				ESX.ShowNotification(_U('you_do_vine') .. aux)
				if aux % 25 == 0 and aux / 25 ~= 0 then
					ESX.ShowNotification(_U('you_do_old_vine') .. math.floor(aux/25))
				end
			elseif action == 'process_juice' then
				ESX.ShowNotification(_U('you_do_raisin_juice') .. aux)
			elseif action == 'señlling_wine' then
				ESX.ShowNotification(_U('you_do_selling_vine') .. aux)
			else
				ESX.ShowNotification(_U('you_do_selling_raisin_juice') .. aux)
			end	
			aux = aux + 1
		end
	end
	FreezeEntityPosition(ped, false)
	qtty = math.floor(index/180)
	TriggerServerEvent('esx_vigneronjob:addQuantity', qtty, action)
end)
