					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					  ...
					  if locked == 1 or class == 15 or class == 16 or class == 14 then
						  ...
						  TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
						   ...
						   ESX.ShowNotification('Este vaul esta ~r~Cerrado')
					  end
					end
				else
					ESX.ShowNotification('no hay ningun ~r~Vehiculo~w~ cerca')
				end
				lastOpen = true
				GUI.Time  = GetGameTimer()
			else
				TriggerEvent('esx:showNotification', "Alguien ya esta mirando el vaul.")
			end
		end, globalplate)
	end
	...
	if inventory ~= nil and #inventory > 0 then
		...
		if inventory[i].type == 'item_standard' then
			...
			elseif inventory[i].type == 'item_weapon' then
			  table.insert(elements, {
				label     = inventory[i].label .. ' | Municion: ' .. inventory[i].count,
				count     = inventory[i].count,
				value     = inventory[i].name,
				type	  = inventory[i].type
			  })	
		...
		for i=1, #weaponList, 1 do
		  ...
		  if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
            ...
            for i=1, #vehiclePlate do
              if all_trim(vehiclePlate[i].plate) == all_trim(GetVehicleNumberPlateText(vehFront)) then
                ownedV = 1
                break
              else
                ownedV = 0
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              local Kgweight =  totalweight/1000
				...
				--  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
					...
					ESX.ShowNotification('Peso del vaul : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
					Citizen.Wait(500)
					TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
				else
					ESX.ShowNotification('Has llegado al límite de ~r~ '..MaxVh..' Kg')
				end
			else
