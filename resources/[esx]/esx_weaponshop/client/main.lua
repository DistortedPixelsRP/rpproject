...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.TriggerServerCallback('esx_weaponshop:buyWeapon', function(bought)
			if bought then
				if data.current.weaponName ~= "clip" then
...
function DisplayBoughtScaleform(weaponName, price)
	...
	PushScaleformMovieMethodParameterString(_U('buy', ESX.Math.GroupDigits(price)))
					...
					if Config.LicenseEnable and Config.Zones[CurrentActionData.zone].Legal then
						ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
							if hasWeaponLicense then
								...
								ESX.ShowNotification("No tienes una licencia de armas, ve a la comisaría para obtenerla.")
...
AddEventHandler('esx_weaponshop:clipcli', function()
  ...
  if IsPedArmed(ped, 4) then
    ...
    if hash~=nil then
      ...
      ESX.ShowNotification("Recargaste el arma.")
    else
      ESX.ShowNotification("Este tipo de municion no es compatible.")
    end
  else
