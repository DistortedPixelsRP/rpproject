...
function ProgressWithStartEvent(action, start, finish)
    ...
    if not IsEntityDead(GetPlayerPed(-1)) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("mythic_base:client:SendAlert", { text = "Ya estas realizando una accion", type = "error", layout = "topRight", timeout = 1500 })
        end
    else
        TriggerEvent("mythic_base:client:SendAlert", { text = "No se puede realizar una acción mientras esté muerto", type = "error", layout = "topRight", timeout = 1500 })
    end
end
