...
local screenPosX = 0.125                    -- X coordinate (top left corner of HUD)
local screenPosY = 0.885                    -- Y coordinate (top left corner of HUD)

-- GENERAL PARAMETERS
local enableController = true               -- Enable controller inputs

-- SPEEDOMETER PARAMETERS
local speedLimit = 120.0                    -- Speed limit for changing speed color
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                drawTxt("CRUCERO", 2, cruiseColor, 0.4, screenPosX + 0.040, screenPosY + 0.048)

                -- Draw seatbelt status if not a motorcyle
                if vehicleClass ~= 8 then
                    local seatbeltColor = seatbeltIsOn and seatbeltColorOn or seatbeltColorOff
                    drawTxt("CINTURON", 2, seatbeltColor, 0.4, screenPosX + 0.080, screenPosY + 0.048)
                end
            end
        end
    end
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            if pedInVeh then
                ...
                if fuelShowPercentage then
                    ...
                    currentFuel = math.ceil(GetVehicleFuelLevel(vehicle))
                end
            end
